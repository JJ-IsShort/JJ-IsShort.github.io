(ns config
  (:require
   ...
   [routing :refer [extract-location]]))
   ...
   {:Projects {:render (fn [state store]
                         (let [page-args (:location/path (:selected-page state))]
                           (if-not page-args
                             ...
                             (if-let [project-page (some #(= (:id %) (keyword page-args)) project-pages)]
                               [:div "Project found"]
                               [:div {:class [:flex :justify-center]}
                                [:div {:class [:flex :flex-col
                                               :gap-4 :w-fit]}
                                 ...
                                 (into [:ul {:class [:list-disc]}]
                                       (map (fn [proj-page] [:li {:on {:click (fn [e]
                                                                                (println "test")
                                                                                (.pushState js/history "" "" (str "#/Projects/" (name (:id proj-page))))
                                                                                (swap! store assoc :selected-page (extract-location
                                                                                                                   (str "#/Projects/" (name (:id proj-page)))
                                                                                                                   page-names)))}}
